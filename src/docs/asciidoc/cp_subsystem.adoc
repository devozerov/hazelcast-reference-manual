...
If a group name is not specified while creating
...
_the DEFAULT CP group_. Besides these 2 predefined CP groups, custom CP groups
can be created at run-time while fetching the CP data structure proxies. For
...
CP members are removed from the CP groups and the CP group majority values are
recalculated.
* When CP Subsystem Persistence is enabled, shutting down CP members are still
kept in the CP groups so they will be a part of the CP group majority calculations.
